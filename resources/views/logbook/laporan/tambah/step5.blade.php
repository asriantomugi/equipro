...
<div class="row mb-2">
  <div class="col-lg-12">
    ...
    <form action="{{ route('tambah.simpanStep5') }}" method="POST" id="formStep5">
      @csrf
      <input type="hidden" name="laporan_id" value="{{ $laporan->id }}">
      <input type="hidden" name="kondisi_layanan_temp" value="{{ $laporan->kondisi_layanan_temp }}">
    ...
    @if (session()->has('notif'))
        @if (session()->get('notif') == 'tambah_sukses')
            $(document).Toasts('create', {
                class: 'bg-success',
                title: 'Sukses!',
                body: 'Laporan telah berhasil disimpan',
                autohide: true,
                delay: 3000
            })
