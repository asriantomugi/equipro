                        ...
                        <form action="{{ route('tambah.simpanStep3Back') }}" method="POST" id="formStep3">
                            ...
                            @if ($laporan->jenis == 1)
                                @foreach ($layanan->daftarPeralatanLayanan as $index => $dpl)
                                    @php
                                        $tl = $tindaklanjutPeralatan[$dpl->peralatan->id][0] ?? null;
                                    @endphp
                                    @if ($index > 0)
                                        <hr>
                                    @endif
                                        ...
                                        <div class="col-sm-9">
                                            <select name="tindaklanjut[{{ $dpl->peralatan->id }}][jenis]" class="form-control tindak-jenis" data-nama="{{ $dpl->peralatan->nama }}">
                                                ...
                                                @foreach ($jenisTindakLanjut as $label => $value)
                                                    <option value="{{ $value ? 1 : 0 }}" {{ $tl && $tl->jenis_tindaklanjut == ($value ? 1 : 0) ? 'selected' : '' }}>{{ ucfirst($label) }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-sm-9">
                                            <textarea name="tindaklanjut[{{ $dpl->peralatan->id }}][deskripsi]" class="form-control">{{ $tl->deskripsi ?? '' }}</textarea>
                                        </div>
                                    </div>
                                        ...
                                        <div class="col-sm-9">
                                            <select name="tindaklanjut[{{ $dpl->peralatan->id }}][kondisi]" class="form-control tindak-kondisi" data-nama="{{ $dpl->peralatan->nama }}">
                                                ...
                                                @foreach ($kondisiTindaklanjut as $label => $value)
                                                    <option value="{{ $value ? 1 : 0 }}" {{ $tl && $tl->kondisi == ($value ? 1 : 0) ? 'selected' : '' }}>{{ ucfirst($label) }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                ...
                                <div class="col-sm-9">
                                    <select name="kondisi_setelah" class="form-control">
                                        ...
                                        @foreach ($kondisiSetelah as $label => $value)
                                            <option value="{{ $value ? 1 : 0 }}" {{ (int) old('kondisi_setelah', $laporan->kondisi_setelah) === ($value ? 1 : 0) ? 'selected' : '' }}>{{ $label }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
