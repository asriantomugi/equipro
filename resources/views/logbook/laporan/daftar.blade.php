...
$(document).ready(function(){
    ...
    // Event listener for button hapus
    $(document).on('click', '.btn-hapus', function(e) {
        e.preventDefault();
        var id = $(this).data('id');
        console.log('Hapus button clicked for ID:', id);
    ...
    @if (session()->has('notif'))
        if (typeof $(document).Toasts === 'function') {
            ...
            console.warn('AdminLTE Toasts not available, using alert instead');
            @if (session()->get('notif') == 'tambah_sukses')
                alert('Sukses: Laporan baru telah berhasil ditambahkan');
            @elseif(session()->get('notif') == 'draft_sukses')
                alert('Sukses: Draft laporan telah berhasil disimpan');
            @elseif(session()->get('notif') == 'hapus_sukses')
                alert('Sukses: Draft laporan telah berhasil dihapus');
            @endif
        }
    @endif
});
...
function konfirmasiHapus(laporan_id) {
    console.log('konfirmasiHapus called with ID:', laporan_id);
    
    if (!laporan_id || laporan_id === undefined || laporan_id === null || laporan_id === '') {
        console.error('ID laporan tidak valid');
        alert('Error: ID laporan tidak valid');
        return;
    }
    ...
    html += '<h4 class="modal-title">Konfirmasi Hapus</h4>';
    html += '<button type="button" class="close" data-dismiss="modal">&times;</button>';
    html += '</div>';
    html += '<div class="modal-body">';
    html += '<p><center>Ingin menghapus draft laporan ini?</center></p>';
    html += '<input type="hidden" name="id" value="'+ laporan_id +'">';
    html += '</div>';
...
console.log('Laporan Detail Script Loaded');
console.log('Available functions: detail(), konfirmasiHapus(), debugAjax(), testDetail(), testModal(), testFormatDate()');
</script>
@endsection
