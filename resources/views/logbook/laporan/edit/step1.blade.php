        ...
        <div class="card">
          ...
          <div class="card-body" id="tabel-daftar-layanan">
            <table id="layananTable" class="table table-bordered table-striped">
              ...
              <tbody>
                {{-- Data akan diisi setelah filter --}}
              </tbody>
            </table>
          </div>
          <div class="card-footer">
    ...
    $('#form-filter-layanan').on('submit', function (e) {
      ...
      $.post('{{ route('logbook.laporan.filter') }}', 
        ...
        function (data) {
          ...
          // Highlight layanan yang sudah dipilih sebelumnya
          let layananTerpilih = '{{ $laporan->layanan_id }}';
          if (layananTerpilih) {
            $('#layananTable tbody tr').each(function(){
              let idLayanan = $(this).data('id');
              if (idLayanan == layananTerpilih) {
