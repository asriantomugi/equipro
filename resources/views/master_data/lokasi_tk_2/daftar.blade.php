  ...
  @if (session()->has('notif'))
    @if (session()->get('notif') == 'tambah_sukses')
      $(document).Toasts('create', {
          class: 'bg-success',
          title: 'Sukses!',
          body: 'Lokasi Tingkat II baru telah berhasil ditambahkan'
        })
    @elseif(session()->get('notif') == 'edit_sukses')
      $(document).Toasts('create', {
          class: 'bg-success',
          title: 'Sukses!',
          body: 'Data Lokasi Tingkat II telah berhasil diubah'
        })
    @elseif(session()->get('notif') == 'tambah_gagal')
      $(document).Toasts('create', {
          class: 'bg-danger',
          title: 'Error!',
          body: 'Gagal menambahkan Lokasi Tingkat II baru'
        })
    @elseif(session()->get('notif') == 'edit_gagal')
      $(document).Toasts('create', {
          class: 'bg-danger',
          title: 'Error!',
          body: 'Gagal mengubah data Lokasi Tingkat II'
        })
    @elseif(session()->get('notif') == 'item_null')
      $(document).Toasts('create', {
          class: 'bg-danger',
          title: 'Error!',
          body: 'Gagal menampilkan data Lokasi Tingkat II'
        })
    @endif
  @endif
</script>
    ...
    $.ajax({
        ...
        success: function(data){
          ...
          var row = '<div class="modal-header">';
              row += '<h4 class="modal-title">Detail Lokasi Tingkat II</h4>';
              ...
              row += "<tr><th>Kode</th><td>:</td><td>"+ data.lokasi_tk_2.kode.toUpperCase(); +"</td></tr>";  
              row += "<tr><th>Nama</th><td>:</td><td>"+ data.lokasi_tk_2.nama.toUpperCase(); +"</td></tr>";
              row += "<tr><th>Lokasi Tingkat I</th><td>:</td><td>"+ data.lokasi_tk_1.kode.toUpperCase();            
              row += " - "+ data.lokasi_tk_1.nama.toUpperCase(); +"</td></tr>";

          if(data.lokasi_tk_2.status == 1){
        ...
        {
            alert('Gagal menampilkan detail Lokasi Tingkat II');
        }
    });
}
</script>
