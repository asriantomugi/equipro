...
@foreach ($daftar as $user)
                    <tr>
                      ...
                      <td>{{ $user->email }}</td>
                      <td>{{ strtoupper($user->role->nama) }}</td>
  @if($user->status == 1)
...
-->
                        <center>
    @if($user->role_id != config('constants.role.super_admin'))
                          <a class="btn btn-info btn-sm" 
                             href="{{url('/master-data/user/edit/'.$user->id)}}" 
                             role="button"
                             title="Edit Data"><i class="fas fa-pencil-alt"></i></a>
							 
						              <a class="btn btn-danger btn-sm" 
                             href="{{url('/master-data/user/password/reset/'.$user->id)}}" 
                             role="button"
    ...
    $.ajax({
        ...
        success: function(data){
          ...
          var row = '<div class="modal-header">';
              row += '<h4 class="modal-title">Detail User Admin</h4>';
